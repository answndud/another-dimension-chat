...
fn pre_network_closeout_moves_to_onion_key_lifecycle_after_backup_exclusion() {
    ...
    let closeout = TransportPreNetworkCloseout::after_backup_exclusion_verification(&verification);
...
fn transport_phase_closeout_selects_onion_hosting_gate_as_next_boundary() {
    ...
    assert_eq!(
        TransportPhaseCloseoutDecision::locked_down(TransportNextRiskBoundary::OnionHostingGate)
            .check(),
    ...
    for (decision, expected) in [
        (
            DescriptorPublicationGateDecision {
                ...
                endpoint_publication_policy: OnionEndpointPublicationPolicy::PairwiseRendezvousOnly,
...
fn inbound_stream_gate_requires_descriptor_publication_gate_and_adapter() {
    ...
    assert_eq!(
        InboundStreamGateDecision::from_publication_gate_and_adapter(gate_ready, &adapter).check(),
...
fn onion_service_key_lifecycle_rejects_plaintext_or_unverified_storage() {
    let backup = TransportBackupExclusionVerification;
    let mut decision = OnionServiceKeyLifecycleDecision::sqlcipher_wrapped_after_unlock(&backup);
...
fn onion_service_key_material_requires_unlock_lifecycle_and_wrapped_record() {
    ...
    assert_eq!(
        OnionServiceKeyMaterialDecision::profile_locked(&key_lifecycle, record_id.clone()).check(),
        Err(OnionServiceKeyMaterialError::ProfileLocked)
    );
    assert_eq!(
...
fn ready_permission_preflight() -> TransportRuntimePermissionPreflight {
    TransportRuntimePermissionPreflight {
        ...
        crash_redaction_policy: TransportCrashRedactionPolicy::SensitivePathsAndIdentifiersRedacted,
...
fn arti_adapter_spike_builds_app_private_config_without_opening_network() {
    ...
    let spike = arti_adapter_spike::ArtiAdapterSpike::fail_closed_app_private_config(dirs.clone())
...
fn profile_scoped_transport_dirs_resolve_app_private_arti_dirs() {
    ...
    let dirs = arti_adapter_spike::ProfileScopedTransportDirs::from_app_data_root(&root, &profile)
...
fn onion_service_launch_adapter_rejects_non_bootstrapped_owner() {
    ...
    assert_eq!(
        ...
        Err(arti_adapter_spike::OnionServiceLaunchAdapterError::PersistentClientNotBootstrapped)
    );
}
...
fn onion_service_launch_adapter_exposes_inbound_stream_boundary() {
    ...
    .expect("launch adapter");
    let inbound = launch_adapter.inbound_stream_boundary(OnionServiceDescriptorPublicationReady);
