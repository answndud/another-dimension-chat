export function renderDaemonBridgeState(state) {
  ...
  const received = `<section class="daemon-invite"><h2>초대 코드 입력</h2><label>상대 릴레이 주소<input id="received-relay-origin" value="${escapeHtml(state.daemonRelayOrigin)}" placeholder="HTTPS 릴레이 주소"></label><label>초대 코드<textarea id="received-invite-code" rows="2" placeholder="상대방에게 받은 코드"></textarea></label><button id="daemon-consume-invite" class="secondary" type="button">초대 확인</button><p class="field-note">서명과 릴레이 접수 증명은 보안 데몬이 자동으로 검증합니다.</p>${state.daemonReceivedInvite ? `<p class="field-note">검증된 계정: ${escapeHtml(state.daemonReceivedInvite.account_id)} · 기기 ${escapeHtml(state.daemonReceivedInvite.device_id)}</p>${state.daemonPairing?.state === "verified" ? '<button id="daemon-approve-pairing" class="primary" type="button">연락처 승인</button><button id="daemon-reject-pairing" class="quiet" type="button">거절</button>' : state.daemonPairing?.state === "established" ? '<p class="verified">연락처 승인이 완료되었습니다.</p>' : ""}` : ""}${state.daemonPairing?.safety_number ? renderDaemonSafetyControls(state.daemonPairing) : ""}</section>`;
