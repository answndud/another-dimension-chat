...
export function renderDaemonContacts(state) {
  ...
  const rows = contacts.length
    ? contacts.map((contact) => `<article class="daemon-contact ${state.daemonSelectedContact === contact.account_id ? "selected" : ""}"><button type="button" class="daemon-contact-select" data-contact-account="${escapeHtml(contact.account_id)}"><strong>${escapeHtml(contact.alias || contact.account_id.slice(0, 18))}${contact.unread_count ? ` <span class="unread-badge">${escapeHtml(contact.unread_count)}</span>` : ""}</strong><small>${escapeHtml(contact.state)} · ${escapeHtml(contact.device_id)}${contact.conversation_id ? ` · ${escapeHtml(contact.conversation_id)}` : " · 대화 미연결"}</small>${contact.last_message_preview ? `<span class="contact-preview">${escapeHtml(contact.last_message_preview)}</span>` : ""}</button><label class="daemon-contact-alias">별칭<input data-contact-alias="${escapeHtml(contact.account_id)}" value="${escapeHtml(contact.alias || "")}" maxlength="128"></label><div class="contact-actions"><button type="button" class="quiet daemon-contact-save" data-contact-save="${escapeHtml(contact.account_id)}">저장</button><button type="button" class="quiet daemon-contact-block" data-contact-block="${escapeHtml(contact.account_id)}">${contact.state === "blocked" ? "차단 해제" : "차단"}</button><button type="button" class="quiet daemon-contact-delete" data-contact-delete="${escapeHtml(contact.account_id)}">삭제</button></div></article>`).join("")
...
export function renderDaemonDevices(state) {
  ...
  return `<section class="daemon-directory"><div class="row-between"><div><h2>연결된 기기</h2><p class="field-note">기기 인증서와 폐기 상태는 로컬 보안 서비스의 암호화 저장소에서 관리됩니다.</p></div><span class="pill">${state.daemonDevices.length}대</span></div><div class="daemon-contact-list">${rows}</div><details class="daemon-device-events"><summary>기기 변경 이력</summary><ul>${eventRows}</ul></details><div class="daemon-device-link"><h3>새 기기 승인</h3><p class="field-note">새 기기에서 만든 연결 요청과 별도 채널로 확인한 일회성 코드를 입력하세요. 개인키는 기기 밖으로 전송되지 않습니다.</p><label>기기 연결 요청<textarea id="daemon-link-request" rows="4" placeholder="새 기기의 연결 요청을 붙여 넣으세요"></textarea></label><label>일회성 승인 코드<input id="daemon-link-code" autocomplete="off" placeholder="별도 채널에서 받은 승인 코드"></label><button id="daemon-link-approve" type="button" class="secondary">기기 승인</button>${state.daemonLinkApproval ? `<label>새 기기로 전달할 승인 자료<textarea readonly rows="4">${escapeHtml(state.daemonLinkApproval)}</textarea></label>` : ""}</div></section>`;
}
...
export function decodeHexText(value) {
  ...
  if (!hex || hex.length % 2 || !/^[0-9a-f]+$/i.test(hex)) throw new Error("보안 서비스가 반환한 평문 형식이 올바르지 않습니다.");
...
export function decodeHexBytes(value) {
  const hex = String(value || "");
  if (!hex || hex.length % 2 || !/^[0-9a-f]+$/i.test(hex)) throw new Error("보안 서비스가 반환한 파일 청크 형식이 올바르지 않습니다.");
...
export function daemonDeliveryLabel(value) {
  return {
    ...
    encrypted: "암호화 완료",
    queued: "전달 대기 중",
    "relay-accepted": "전달 경로 접수됨 · 상대 수신 여부 확인 불가",
    "recipient-received": "이 기기에서 수신됨",
    decrypted: "이 기기에서 복호화·저장됨",
...
export function renderDaemonSessionPanel(state) {
  ...
  const sessionSetup = selectedContact && !conversationId
    ? `<section class="daemon-session-setup"><strong>암호화 연결을 준비하는 중입니다.</strong><p class="field-note">초대 승인 후 보안 서비스가 대화 연결을 자동으로 준비합니다. 연결이 끝나면 이 화면에서 메시지를 보낼 수 있습니다.</p></section>`
    : !selectedContact
      ...
      : `<p class="field-note daemon-session-ready">이 연락처의 로컬 대화가 준비되었습니다.</p>`;
  const deliveryState = state.daemonDeliveryState
    ...
    : "";
  return `<section class="daemon-session-tools daemon-conversation-panel"><header class="daemon-conversation-heading"><div><span class="eyebrow">암호화 대화</span><h2>${escapeHtml(contactTitle)}</h2><p class="field-note">브라우저는 입력과 표시만 담당하고, 암호화 키와 대화 상태는 로컬 보안 서비스가 보관합니다.</p></div>${selectedContact ? `<span class="pill">${escapeHtml(selectedContact.state)}${selectedContact.conversation_id ? " · 연결됨" : " · 준비 필요"}</span>` : ""}</header>${sessionSetup}<input id="daemon-conversation-id" type="hidden" value="${escapeHtml(conversationId)}"><input id="daemon-peer-inbox-url" type="hidden" value="${escapeHtml(peerInboxUrl)}"><div class="daemon-message-list" aria-live="polite" aria-label="대화 메시지">${receivedMessages}</div><label>메시지<textarea id="daemon-message" dir="auto" rows="3" placeholder="메시지를 입력하세요" ${conversationId ? "" : "disabled"}></textarea><label>메시지 보존 기간<select id="daemon-message-expiry"><option value="0">메시지 만료 없음</option><option value="3600">1시간 후 만료</option><option value="86400">24시간 후 만료</option><option value="604800">7일 후 만료</option></select></label><button id="daemon-message-send" class="primary" type="button" ${conversationId ? "" : "disabled"}>보내기</button><section class="daemon-attachment"><h3>파일 보내기</h3><p class="field-note">파일은 로컬 보안 서비스에서 암호화한 뒤 전달됩니다. 상대방의 전달 경로는 선택한 연락처에서 자동으로 사용합니다.</p><label>암호화할 파일<input id="daemon-attachment-file" type="file" ${conversationId ? "" : "disabled"}></label><button id="daemon-attachment-send" class="secondary" type="button" ${conversationId ? "" : "disabled"}>파일 암호화·전송</button>${state.daemonAttachmentState ? `<p class="delivery-state" role="status">${escapeHtml(state.daemonAttachmentState)} · ${state.daemonAttachmentProgress}%</p><progress max="100" value="${state.daemonAttachmentProgress}">${state.daemonAttachmentProgress}%</progress>` : ""}${attachmentRetry}</section>${deliveryState}<section class="daemon-delivery"><h3>받은 메시지</h3><p class="field-note">내 주소가 설정된 경우 새 암호화 봉투를 확인하고 로컬 저장소에 반영합니다.</p><input id="daemon-inbox-url" type="hidden" value="${escapeHtml(state.daemonInboxUrl)}"><button id="daemon-history-load" class="quiet" type="button">로컬 대화 기록 불러오기</button><button id="daemon-history-older" class="quiet" type="button" ${state.daemonMessagesHasMore ? "" : "hidden"}>이전 대화 기록 더 불러오기</button><button id="daemon-delivery-sync" class="secondary" type="button" ${conversationId ? "" : "disabled"}>받은 메시지 동기화</button></section></section>`;
}
