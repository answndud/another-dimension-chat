export function renderDaemonBridgeState(state) {
  ...
  const invite = state.daemonInvite ? `<section class="daemon-invite"><h2>내 초대 코드</h2><p>아래 코드만 상대방에게 별도 채널로 전달하세요. 10분 뒤 만료되고 한 번만 사용할 수 있습니다.</p><label>초대 코드<textarea readonly rows="2">${escapeHtml(state.daemonInvite.invite_code)}</textarea></label><div class="daemon-action-row"><button id="daemon-copy-invite" class="secondary" type="button">코드 복사</button><button id="daemon-revoke-invite" class="quiet" type="button">초대 폐기</button></div></section>` : "";
  const received = `<section class="daemon-invite"><h2>초대 코드 입력</h2><label>상대 릴레이 주소<input id="received-relay-origin" value="${escapeHtml(state.daemonRelayOrigin)}" placeholder="HTTPS 릴레이 주소"></label><label>초대 코드<textarea id="received-invite-code" rows="2" placeholder="상대방에게 받은 코드"></textarea></label><button id="daemon-consume-invite" class="secondary" type="button">초대 확인</button><p class="field-note">서명과 릴레이 접수 증명은 보안 데몬이 자동으로 검증합니다.</p>${state.daemonReceivedInvite ? `<p class="field-note">검증된 계정: ${escapeHtml(state.daemonReceivedInvite.account_id)} · 기기 ${escapeHtml(state.daemonReceivedInvite.device_id)}</p>${state.daemonPairing?.state === "verified" ? '<button id="daemon-approve-pairing" class="primary" type="button">연락처 승인</button><button id="daemon-reject-pairing" class="quiet" type="button">거절</button>' : state.daemonPairing?.state === "established" ? '<p class="verified">연락처 승인이 완료되었습니다.</p>' : ""}` : ""}</section>`;
