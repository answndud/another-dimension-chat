use super::{
    ...
    encode_message_payload, hex_bytes, hex_decode, json_escape, json_string, json_u64,
    mls_device_credential, pairing_ready, persist_message, response, AttachmentDescriptor, Request,
    RouteContext, MAX_MESSAGE_TTL_SECONDS,
};
...
pub(crate) fn handle_mls_route(
    ...
    let reply = match (request.method, request.path) {
        ("POST", "/local-api/session/remove-device") => {
            ...
            let commits =
                match catalog.remove_device(&mls_device_credential(account_id, device_id), store) {
                    Ok(commits) => commits,
