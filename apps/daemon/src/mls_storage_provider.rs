//! Direct OpenMLS StorageProvider implementation for the daemon boundary.
//!
//! Values remain bounded, namespaced, and held behind the daemon-owned store
//! boundary. The provider intentionally has no browser-facing serialization API.
#![allow(clippy::too_many_arguments)]
#![allow(unexpected_cfgs)]

use openmls_traits::storage::{traits, Entity, StorageProvider};
use serde::Serialize;
...
pub struct EncryptedMlsStorage {
...
pub enum EncryptedMlsStorageError {
...
impl std::error::Error for EncryptedMlsStorageError {}
...
impl StorageProvider<CURRENT_VERSION> for EncryptedMlsStorage {
    type Error = EncryptedMlsStorageError;
...
fn epoch_key_pairs_id(
    ...
    leaf_index: u32,
) -> Result<Vec<u8>, <EncryptedMlsStorage as StorageProvider<CURRENT_VERSION>>::Error> {
...
impl From<serde_json::Error> for EncryptedMlsStorageError {
