...
test("message composer routes send, receive, retry, and route preparation through explicit helpers", () => {
  ...
  assert.match(functionBody(mainJs, "startProductionTwoProfileOnionReceive"), /privateRouteCodeReadyForReceive/);
  assert.match(functionBody(mainJs, "startProductionTwoProfileOnionReceive"), /receiveNotStartedAfterRouteCode/);
  ...
  assert.match(functionBody(mainJs, "preparePrivateDeliveryRoute"), /showLatestRetryableOutboundNotice\(input\)/);
...
test("failed sends stay recoverable from the chat flow", () => {
  ...
  assert.match(functionBody(mainJs, "clearStaleSendRecoveryNotice"), /latestTwoProfileRetryableOutboundEntry\(input\)/);
