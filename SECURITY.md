...
Default-build production code now includes narrow decision boundaries for pairing, session setup, envelope handling, replay rejection, storage policy tests, a SQLCipher-backed storage spike, and passphrase unlock tests. These are implementation guardrails, not a secure messenger release.
