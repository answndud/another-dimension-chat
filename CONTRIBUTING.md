...
The unsigned public beta release upload set is generated by:

```bash
scripts/prepare_unsigned_public_beta_release.sh
```

The generated folder is ignored and must not be committed. Upload only the
generated files listed in `reference/BETA_RELEASE_CHECKLIST.md`.
