...
| Message crypto | Noise XX smoke and envelope-session guardrails exist; no reviewed production messaging protocol or ratchet is integrated. | Select and review the production session model, define persistence expectations, add vectors/tests, and keep placeholder crypto behind `dev-insecure`. |
| Transport | High-risk policy, fail-closed onion transport, Arti bootstrap/lifecycle spikes, and descriptor/stream/envelope-I/O gates exist. | Implement bounded Tor/onion lifecycle only behind explicit phase gates, keep direct P2P out of high-risk mode, and prove no silent fallback to direct routes. |
| Local storage | SQLCipher-backed `ADREC1` spike, record classification, passphrase unlock, replay-state persistence, and deletion helpers exist. | Define production key management, key wrapping, backup exclusion, rollback handling, migration, and durable session/message persistence before claiming storage readiness. |
...
- [CRYPTO_DECISION.md](CRYPTO_DECISION.md): narrow the production session/ratchet/library decision.
