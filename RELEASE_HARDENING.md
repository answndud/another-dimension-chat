...
- A release signing implementation plan in [RELEASE_SIGNING_PLAN.md](RELEASE_SIGNING_PLAN.md) that records the intended offline detached-signature path without claiming signing readiness.
...
| --- | --- | --- |
| Release signing | No signing workflow or signed artifact verification exists; a pre-implementation signing plan exists. | Add platform signing, verification instructions, and a checked release process. |
