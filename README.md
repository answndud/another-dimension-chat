...
> **앱 코드 공급망 경고:** 현재 사용자 소유 서버는 편의를 위해 브라우저
> JavaScript/WASM도 제공합니다. 서버나 reverse proxy가 변조되면 암호화가 시작되기
> 전에 passphrase·평문·키가 탈취될 수 있습니다. 현재 release에는 이 문제를
> 해결하는 독립 서명·검증 체계가 없으므로, 이것만으로 고위험 통신에 사용하면
> 안 됩니다.
...
2. **사용자 소유 서버** — 사용자의 기기에서 직접 실행합니다. 서버는 정적 웹
   UI를 제공하고, 상대 서버에서 온 암호화 봉투를 잠시 보관·전달합니다.
