...
| --- | --- | --- |
| Release signing | `RELEASE_HARDENING.md` records a pre-implementation signing plan, OpenSSL-compatible tooling decision, ceremony dry-run record, dry-run verifier, and disposable detached-signature fixture, but no release signing workflow or signed artifact verification exists. `scripts/verify_release_artifact_gates.sh`, `scripts/verify_release_signing_plan.sh`, `scripts/verify_release_signing_dry_run.sh`, `scripts/verify_release_detached_signature_fixture.sh`, `scripts/verify_release_signing_tooling_gate.sh`, and `scripts/verify_release_signing_ceremony_dry_run.sh` confirm this gate is incomplete. | Not satisfied. |
