...
| --- | --- | --- |
| Release signing | `RELEASE_HARDENING.md` records a pre-implementation signing plan and dry-run verification fixture, but no real signing workflow or signed artifact verification exists. `scripts/verify_release_artifact_gates.sh`, `scripts/verify_release_signing_plan.sh`, and `scripts/verify_release_signing_dry_run.sh` confirm this gate is incomplete. | Not satisfied. |
...
The next implementation slice should replace the release-signing dry-run marker with a real detached-signature tool in fixture-only verification.
