...
아래는 실제 ciphertext가 아니라 설명용 fake envelope다.

```text
ADENV1|1|room-FAKE|7|data|FAKE_PADDED_CIPHERTEXT_HEX
```

읽는 방법:

- `ADENV1`: envelope format marker
- `1`: protocol version
- `room-FAKE`: channel id
- `7`: message number
- `data`: message type
- `FAKE_PADDED_CIPHERTEXT_HEX`: padding bucket에 맞춘 ciphertext

replay window가 이미 message number `7`을 성공적으로 받은 상태라면 같은 envelope는 duplicate로 거부되어야 한다. 하지만 message number `7`의 tampered ciphertext가 먼저 들어와 decrypt에 실패했다면, replay state는 아직 `7`을 받은 것으로 commit되면 안 된다. 그래야 나중에 정상 ciphertext `7`이 왔을 때 받을 수 있다.
...
- [crates/protocol/src/lib.rs](../../crates/protocol/src/lib.rs): `Envelope`
- [crates/protocol/src/lib.rs](../../crates/protocol/src/lib.rs): `pad_to_bucket`
- [crates/protocol/src/lib.rs](../../crates/protocol/src/lib.rs): `ReplayWindow`
- [crates/protocol/src/lib.rs](../../crates/protocol/src/lib.rs): `accept_after_decrypt`
- [crates/protocol/src/lib.rs](../../crates/protocol/src/lib.rs): `encode_state` / `decode_state`
- [crates/core/src/lib.rs](../../crates/core/src/lib.rs): production receive/replay persistence 관련 tests
