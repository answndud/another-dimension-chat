...
이 폴더는 Another Dimension Chat에 사용된 보안, 통신, 저장소, release 개념을 초보자도 따라올 수 있게 설명하는 Korean public learning guide다.

English readers can use this as a source-linked Korean learning appendix. The
project overview and public release boundary are still maintained in English in
...
이 guide는 tutorial이다. Release body, SECURITY policy, production claim gate,
High-Risk readiness claim을 대체하지 않는다. README/SECURITY/reference gate가
허용하지 않는 claim은 이 guide에서도 열리지 않는다.
...
이 guide에서 프로젝트 구조를 빠르게 이해하려면 아래 순서로 읽는다.

1. [02-central-trust-and-metadata.md](./02-central-trust-and-metadata.md):
   중앙 account/contact/message/backup authority를 줄이는 이유.
2. [04-pairing-and-safety-verification.md](./04-pairing-and-safety-verification.md):
   pairwise invite와 safety material을 사람이 확인해야 하는 이유.
3. [05-encryption-envelope-and-replay.md](./05-encryption-envelope-and-replay.md):
   encrypted envelope와 replay window가 분리되는 이유.
4. [07-transport-manual-onion-and-network-boundaries.md](./07-transport-manual-onion-and-network-boundaries.md):
   manual envelope default path와 explicit/fail-closed onion path의 tradeoff.
5. [08-tauri-shell-and-rust-core-boundary.md](./08-tauri-shell-and-rust-core-boundary.md):
   Rust core가 보안 의미를 소유하고 Tauri shell은 redacted state와 explicit action만 다루는 이유.

현재 threat-model 설명에서는 remote passive observer, remote active attacker,
malicious peer, local-at-rest attacker, supply-chain/update attacker를 mitigation
target으로 다룬다. compromised endpoint, direct coercion, full global traffic
correlation은 app 단독으로 보장하지 못하므로 `not_protected`로 남긴다.
