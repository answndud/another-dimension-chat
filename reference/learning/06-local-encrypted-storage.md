...
- `crates/storage/src/lib.rs`: `ProductionRecordKind`
- `crates/storage/src/lib.rs`: `StorageProtection`
- `crates/storage/src/lib.rs`: `ReplayRollbackProtection`
- `crates/storage/src/lib.rs`: `SqlCipherRecordStore`
- `crates/storage/src/lib.rs`: `unlock_with_passphrase`
- `crates/storage/src/lib.rs`: `rekey_with_passphrase`
- `crates/storage/src/lib.rs`: delete helpers

Core 연결:

- `crates/core/src/lib.rs`: message/session/replay state를 storage record로 저장하는 flow

Desktop 연결:

- `apps/desktop-tauri/src-tauri/src/lib.rs`: desktop platform boundary, redacted diagnostics, local data lifecycle controls
