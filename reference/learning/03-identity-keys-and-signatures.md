...
아래 값들은 실제 key나 signature가 아니라 설명용 가짜 값이다.

```text
payload:
  owner_profile: "alice"
  pairing_nonce: "FAKE_NONCE_123"
  pairwise_public_key: "FAKE_PUBLIC_KEY_ALICE"
  rendezvous_endpoint: "manual-envelope-only"
  ttl_seconds: 600

canonical_bytes:
  "another-dimension|pairing-payload-v1|alice|FAKE_NONCE_123|..."

signature:
  "FAKE_SIGNATURE_OVER_CANONICAL_BYTES"
```

검증자가 확인할 수 있는 것:

- signature가 `canonical_bytes` 위에 만들어졌는지
- payload가 서명 후 바뀌지 않았는지
- public key 형식이 pairing payload와 맞는지

검증자가 이것만으로 확인할 수 없는 것:

- `FAKE_PUBLIC_KEY_ALICE`가 현실의 Alice가 가진 key인지
- Alice의 device가 compromise되지 않았는지
- 사용자가 safety verification을 실제로 수행했는지
...
[crates/identity](../../crates/identity)는 profile/contact/key/signature type을 다룬다.

[crates/pairing](../../crates/pairing)은 pairing payload에 public key, signature, nonce, endpoint policy, protocol capabilities 등을 넣고 canonical bytes에 signature를 붙인다.
...
- [crates/identity/src/lib.rs](../../crates/identity/src/lib.rs): `ProfileName`, `ContactId`, `PairwisePublicKey`, `ProductionPairwisePrivateKey`
- [crates/pairing/src/lib.rs](../../crates/pairing/src/lib.rs): `PairingPayload`, `canonical_bytes`, `production_pairing_payload_for`, `production_pairing_nonce`
