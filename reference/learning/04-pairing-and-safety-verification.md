...
아래는 설명용 가짜 pairing 상황이다.

```text
Alice sees:
  remote_profile: "bob"
  remote_pairwise_key: "FAKE_KEY_BOB_A"
  safety_phrase: "river-copper-lamp-42"

Bob sees:
  remote_profile: "alice"
  remote_pairwise_key: "FAKE_KEY_ALICE_A"
  safety_phrase: "river-copper-lamp-42"
```

이 경우 두 사용자가 별도 통화나 직접 만남 같은 out-of-band 경로로 같은 safety phrase를 확인하면, 둘이 같은 cryptographic context를 보고 있을 가능성이 높아진다.

반대로 Bob 화면에 아래처럼 나온다면 멈춰야 한다.

```text
Bob sees:
  safety_phrase: "river-copper-lamp-99"
```

signature verification이 성공하더라도 safety phrase가 다르면 중간자 공격, payload 교체, 잘못된 invite, stale pairing 같은 문제를 의심하고 re-pairing해야 한다.
...
- [crates/pairing/src/lib.rs](../../crates/pairing/src/lib.rs): `PairingPayload`
- [crates/pairing/src/lib.rs](../../crates/pairing/src/lib.rs): `canonical_bytes`
- [crates/pairing/src/lib.rs](../../crates/pairing/src/lib.rs): `production_pairing_payload_for`
- [crates/pairing/src/lib.rs](../../crates/pairing/src/lib.rs): `transcript`
- [crates/pairing/src/lib.rs](../../crates/pairing/src/lib.rs): `verify_pairing_signature`
- [crates/core/src/lib.rs](../../crates/core/src/lib.rs): pairing session prepare/save/status 관련 summary
