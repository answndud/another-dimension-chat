...
처음부터 `crates/core/src/lib.rs` 같은 큰 파일을 열면 어렵다. 먼저 개념 지도를 만들고, 그 다음 source file을 연결해서 보는 편이 좋다.
...
| 순서 | 파일 | 먼저 볼 이유 |
| --- | --- | --- |
| 1 | `README.md` | public beta 상태와 non-claim을 먼저 확인 |
| 2 | `SECURITY.md` | 현재 보안 policy와 제한 확인 |
| 3 | `crates/protocol/src/lib.rs` | envelope/replay 개념이 비교적 작게 모여 있음 |
| 4 | `crates/pairing/src/lib.rs` | pairing payload와 safety transcript 확인 |
| 5 | `crates/identity/src/lib.rs` | profile/contact/key/signature type 확인 |
| 6 | `crates/storage/src/lib.rs` | local encrypted storage boundary 확인 |
| 7 | `crates/transport/src/lib.rs` | fail-closed transport policy 확인 |
| 8 | `crates/core/src/lib.rs` | 여러 crate가 product flow로 조합되는 방식 확인 |
| 9 | `apps/desktop-tauri/src-tauri/src/lib.rs` | Tauri command/Rust boundary 확인 |
| 10 | `apps/desktop-tauri/src/main.js` | UI가 redacted status와 explicit action을 다루는 방식 확인 |
