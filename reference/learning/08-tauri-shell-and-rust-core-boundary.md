...
아래는 shell과 core 책임을 나누는 방식의 설명용 예시다.

| 사용자 행동 | Shell이 할 일 | Rust core가 결정할 일 |
| --- | --- | --- |
| `Unlock` 버튼 클릭 | passphrase input을 command로 전달 | store unlock 성공/실패와 redacted error 반환 |
| `Export envelope` 클릭 | 저장 dialog와 copy UI 제공 | envelope 생성, message number, replay/session state 처리 |
| `Try onion path` 클릭 | permission checkbox 상태 전달 | preflight, fail-closed, transport policy 판단 |
| `Copy diagnostics` 클릭 | text를 clipboard에 복사 | raw secret 없는 redacted summary 생성 |

초보자가 읽을 때 핵심은 UI가 "가능해 보이게" 만드는 것과 core가 "허용해도 되는 상태인지" 판단하는 것이 다르다는 점이다.
...
- [apps/desktop-tauri/src-tauri/src/lib.rs](../../apps/desktop-tauri/src-tauri/src/lib.rs): `DESKTOP_PLATFORM_BOUNDARY_POLICIES`
- [apps/desktop-tauri/src-tauri/src/lib.rs](../../apps/desktop-tauri/src-tauri/src/lib.rs): `desktop_platform_boundary_summary`
- [apps/desktop-tauri/src/main.js](../../apps/desktop-tauri/src/main.js): `manualNetworkPermissionEnabled`
- [apps/desktop-tauri/src/main.js](../../apps/desktop-tauri/src/main.js): public diagnostics summary
- [crates/core/src/lib.rs](../../crates/core/src/lib.rs): app-facing summary types and orchestration functions
