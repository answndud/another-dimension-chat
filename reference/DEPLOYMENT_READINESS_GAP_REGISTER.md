...
| Message-content E2EE | Supported local/manual 1:1 envelope message-content scope is source-gated. | Broad production E2EE readiness remains false. |
...
1. D100-1: replace broad production E2EE false with a pass-capable source gate
   for the actual protocol/session implementation surface, then keep external
   review false until review happens.
2. D100-2: replace broad production key-management false with a pass-capable
...
3. D100-3: prepare signed/notarized macOS RC execution so it passes
   automatically when Developer ID and notary credentials appear.
4. D100-4: make the Windows public artifact path executable on a real Windows
   runner/device and reject local-only results.
5. D100-5: run external review, field evidence, and representative usability
