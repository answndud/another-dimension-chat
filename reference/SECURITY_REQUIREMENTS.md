...
| `ARCH-04` | The daemon, browser bridge, and relay have separate ownership and failure domains. | `partial` | Daemon boundary contract, IPC negative fixtures, and release archive separation. |
...
| `AUTH-04` | Account Root Key signs device certificates; unsigned, revoked, or duplicate devices cannot enter a session. | `partial` | Fixed identity/device vectors, revocation fixture, and redacted device-event artifact. |
...
| `DATA-04` | Long-term private state is owned by the daemon, not browser storage; keychain-unavailable and recovery failure paths fail closed. | `partial` | Storage ownership scan, keychain failure fixture, corrupt/old snapshot fixture, and redacted recovery artifact. |
...
| `BRIDGE-01` | A hostile web origin cannot bootstrap, replay, or widen a daemon session. | `partial` | Cross-origin, DNS-rebinding, token-replay, stale-cookie, restart, and version-mismatch fixtures. |
| `BRIDGE-02` | The UI receives only least-privilege daemon results and never reads private keys, raw DB files, or ratchet state. | `partial` | Static boundary scan, API contract test, and redacted DOM/network artifact. |
