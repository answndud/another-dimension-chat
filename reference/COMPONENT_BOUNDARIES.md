...
| Release and updates | Unsigned public beta release path exists with DMG SHA-256, provenance copy, manual update-integrity note, dependency lockfile hash baseline, public threat model, and independent review packet. It still has no signing, notarization, reproducible/equivalent build proof, SBOM, dependency audit, completed external review, or auto-update channel. | Keep release evidence public-safe and do not claim secure messenger readiness until external review and final claim gates are satisfied. |
