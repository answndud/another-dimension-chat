...
- `SessionDurableStateAdapterNonReadinessGuard` keeps rollback protection, product store writes, durable session persistence, production E2EE readiness, durable Noise transport persistence, and runtime messaging false.
- The session durable-state store-write adapter writes caller-supplied prepared sealed records through an already-unlocked `SqlCipherRecordStore` only after kind, scope, and record-id prefix binding checks while preserving the production non-readiness guard.
- `SessionDurableStateStoreWriteStatusMirror` reports that the store-write adapter boundary exists and keeps production store write, unlock command, durable session persistence, rollback protection, and runtime messaging disabled.
...
- `ProductionLocalDataLifecyclePolicySummary` fixes the current v0.1 local data policy: passphrase-first unlock is required, OS keystore wrapping is optional and non-required, rollback prevention and secure media deletion remain explicit non-claims, backup exclusion is required as best-effort policy but not verified by this boundary, schema migration is forward-only, and prototype data migration is not ready.
- `ProductionBackupMigrationBoundarySummary` fixes the public beta backup/migration boundary: local backup-exclusion verification is required, cloud backup/sync and backup recovery are not provided, schema migration is forward-only, destructive migration is blocked, rollback detection is marker-only, and rollback prevention plus secure media deletion remain non-claims.
...
| Pairing and safety | Production pairing payload and safety material boundaries exist; local demo uses development flow for ergonomics. | Keep invite-code pairing as the first setup path, bind identity/prekey/endpoint/capability material into safety display, and reject mixed dev/production schemes. |
...
| Local storage | SQLCipher-backed `ADREC1` spike, record classification, passphrase-first unlock, replay-state persistence, deletion helpers, forward-only schema versioning, `ProductionLocalDataLifecyclePolicySummary`, and `ProductionBackupMigrationBoundarySummary` exist. | Keep production key wrapping, rollback protection, cloud backup/sync, backup recovery, secure media deletion, verified backup exclusion, and prototype data migration as non-claims until separately implemented and reviewed. |
...
| Session encrypted-record adapter spike | `SessionDurableStateEncryptedRecordAdapter` prepares allowed sealed records for session durable state. | Preserve session-transport in-memory and non-readiness invariants before wiring it into user-facing flows. |
...
| Session store-write adapter boundary | `SessionDurableStateStoreWriteAdapter` writes prepared sealed durable-state records through a caller-supplied unlocked `SqlCipherRecordStore` after kind, scope, and record-id prefix binding checks. | Do not expose user-facing unlock or runtime messaging until key wrapping, backup exclusion, rollback behavior, and session lifecycle are reviewed. |
| Session store-write status mirror | `SessionDurableStateStoreWriteStatusMirror` exposes the adapter boundary while keeping production store-write readiness flags false. | Keep this mirror aligned with any future store-write changes before exposing user-facing unlock or runtime messaging. |
...
| Release and updates | Unsigned public beta release path exists with DMG SHA-256, provenance copy, manual update-integrity note, dependency lockfile hash baseline, public threat model, independent review packet, public intake policy, repository governance, and diagnostics redaction boundary. It still has no signing, notarization, reproducible/equivalent build proof, SBOM, dependency audit, completed external review, auto-update channel, crash upload, telemetry, or raw log export. | Keep release evidence public-safe and do not claim secure messenger readiness until external review and final claim gates are satisfied. |
...
2. **Storage/key management boundary**: implement optional key wrapping/KDF parameters only after the passphrase-first policy, rollback non-claim, logical deletion, backup-exclusion best-effort, and forward-only migration constraints remain intact.
