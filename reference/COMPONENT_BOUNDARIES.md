...
- The session durable-state store-write adapter writes caller-supplied prepared sealed records through an already-unlocked `SqlCipherRecordStore` while preserving the production non-readiness guard.
...
| Session store-write adapter boundary | `SessionDurableStateStoreWriteAdapter` writes prepared sealed durable-state records through a caller-supplied unlocked `SqlCipherRecordStore`. | Do not expose user-facing unlock or runtime messaging until key wrapping, backup exclusion, rollback behavior, and session lifecycle are reviewed. |
