...
usage() {
  cat <<'EOF'
사용법:
  update_local_server.sh --install-root DIR --archive DIR --public-key PEM --trust-manifest JSON --trust-manifest-key PEM --review-signoff JSON --reviewer-public-key PEM [--min-version VERSION] [--stop]
...
[ -n "$review_signoff" ] && [ -n "$reviewer_public_key" ] || { echo "update에는 독립 보안 검토 sign-off와 reviewer public key가 모두 필요합니다." >&2; usage >&2; exit 2; }
verify
if is_running; then
...
if [ -n "$min_version" ]; then
  sh "$install_root/scripts/install_local_server.sh" --archive "$archive" --public-key "$public_key" --trust-manifest "$trust_manifest" --trust-manifest-key "$trust_manifest_key" --review-signoff "$review_signoff" --reviewer-public-key "$reviewer_public_key" --destination "$stage" --data-dir "$data_dir" --min-version "$min_version"
else
  sh "$install_root/scripts/install_local_server.sh" --archive "$archive" --public-key "$public_key" --trust-manifest "$trust_manifest" --trust-manifest-key "$trust_manifest_key" --review-signoff "$review_signoff" --reviewer-public-key "$reviewer_public_key" --destination "$stage" --data-dir "$data_dir"
fi
