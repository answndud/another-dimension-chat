...
DOC="$ROOT_DIR/RELEASE_SIGNING_CANDIDATE_EVIDENCE_PACKAGE_CHECKSUM_NON_CLAIM_GUARD.md"

test -f "$DOC"

grep -q 'Guard status: checksum non-claim guard only, not release evidence' "$DOC"
grep -q 'Checksum and digest coverage remains fixture-only' "$DOC"
...
grep -q 'RELEASE_SIGNING_CANDIDATE_EVIDENCE_PACKAGE_CHECKSUM_NON_CLAIM_GUARD.md' "$ROOT_DIR/README.md"
grep -q 'RELEASE_SIGNING_CANDIDATE_EVIDENCE_PACKAGE_CHECKSUM_NON_CLAIM_GUARD.md' "$ROOT_DIR/RELEASE_SIGNING_PLAN.md"
grep -q 'checksum non-claim guard' "$ROOT_DIR/RELEASE_HARDENING.md"
...
if grep -R -n -E 'release artifact checksums verified|release artifact hashes verified|real release artifact hashes compared|detached signatures verified|signed artifact verification passed|artifact authenticity proven|candidate evidence collected|release signing approved|release candidate approved|release hardening gates passed|v0\.1-security-ready approved' \
  ...
  "$ROOT_DIR/RELEASE_SIGNING_CANDIDATE_EVIDENCE_PACKAGE_CHECKSUM_AUDIT.md" \
  "$ROOT_DIR/RELEASE_SIGNING_CANDIDATE_EVIDENCE_PACKAGE_CHECKSUM_NON_CLAIM_GUARD.md" \
  "$ROOT_DIR/COMPONENT_BOUNDARIES.md" >/dev/null; then
  echo "checksum fixture coverage must not imply release evidence or approval" >&2
  exit 1
fi

printf 'release signing candidate evidence package checksum non-claim guard remains guard-only\n'
