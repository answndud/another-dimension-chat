...
grep -q 'Release signing | `RELEASE_HARDENING.md` records a pre-implementation signing plan, OpenSSL-compatible tooling decision, ceremony dry-run record, ceremony command harness, dry-run verifier, and disposable detached-signature fixture, but no release signing workflow or signed artifact verification exists' \
  "$ROOT_DIR/RELEASE_COMPLETION_AUDIT.md"
