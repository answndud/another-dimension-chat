...
cp "$PROJECT_DIR/scripts/install_client.sh" "$PROJECT_DIR/scripts/verify_client_release_gate.mjs" "$PROJECT_DIR/scripts/verify_release_manifest.mjs" "$PROJECT_DIR/scripts/release_manifest.mjs" "$PROJECT_DIR/scripts/verify_web_artifact.mjs" "$PROJECT_DIR/scripts/verify_release_trust.mjs" "$ROOT/scripts/"
...
node -e 'const fs=require("node:fs"); const [file,commit,version,epoch]=process.argv.slice(1); fs.writeFileSync(file, JSON.stringify({format:"another-dimension-client-provenance",version,sourceCommit:commit,sourceDateEpoch:Number(epoch)}, null, 2)+"\n")' "$ROOT/RELEASE-PROVENANCE.json" "$(git -C "$PROJECT_DIR" rev-parse HEAD)" "$VERSION" "$SOURCE_DATE_EPOCH"
