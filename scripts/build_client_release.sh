...
cp "$PROJECT_DIR/scripts/install_client.sh" "$ROOT/scripts/"
...
printf '{\n  "format": "another-dimension-client-provenance",\n  "version": "%s",\n  "sourceCommit": "%s",\n  "sourceDateEpoch": %s\n}\n' "$VERSION" "$(git -C "$PROJECT_DIR" rev-parse HEAD)" "$SOURCE_DATE_EPOCH" > "$ROOT/RELEASE-PROVENANCE.json"
"$TOOLS" release-manifest create --root "$ROOT" --version "$VERSION" --private-key "$AD_RELEASE_SIGNING_KEY"
"$TOOLS" release-manifest verify --root "$ROOT" --public-key "$AD_RELEASE_PUBLIC_KEY"
