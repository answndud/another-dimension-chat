...
if (!/daemon-owned-openmls-messaging-path-active-development/.test(library) || !/High-risk release remains disabled/.test(main)) failures.push("daemon readiness marker is missing");
...
if (boundary.candidateProductPath !== "apps/daemon" || !String(boundary.supportedProduct).includes("local OpenMLS daemon")) failures.push("product boundary does not select the daemon product");
if (boundary.highRiskAllowed !== false) failures.push("high-risk mode must remain disabled");
if (failures.length) { console.error(failures.join("\n")); process.exit(1); }
console.log("daemon boundary passed: sole Rust workspace product, no legacy crypto dependencies, high-risk disabled");
