...
ACCEPTED_ITEMS="invite#create#join#verify#send#export#import#reply#receive#retry#cancel#delete#unlock#reopen#diagnostics#release-non-claim"
...
require_text "$MAIN_JS" "async function createInviteCode"
require_text "$MAIN_JS" "async function startInviteRoomFromCode"
require_text "$MAIN_JS" "async function openInviteRoomFromToken"
...
require_text "$MAIN_JS" "async function exportProductionMessageEnvelope"
require_text "$MAIN_JS" "async function importProductionMessageEnvelope"
require_text "$MAIN_JS" "async function exportProductionReceivedMessage"
require_text "$MAIN_JS" "function replyToLatestTwoProfileMessage"
...
require_text "$MAIN_JS" "async function loadProductionTwoProfileTranscript"
...
require_text "$UI_SMOKE" "saved rooms can be listed and reopened"
require_text "$UI_SMOKE" "room list controls are wired to room flow instead of settings"
...
require_text "$UI_SMOKE" "join failure explains when the invite room is not open"
...
require_text "$UI_SMOKE" "exportProductionMessageEnvelope"
require_text "$UI_SMOKE" "exportProductionReceivedMessage"
require_text "$UI_SMOKE" "click Show plaintext before writing the reply"
...
require_text "$ACTION_STATE_TEST" "room list metadata follows latest imported conversation entry"
