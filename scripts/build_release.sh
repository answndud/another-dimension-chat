...
cp "$PROJECT_DIR/reference/PRODUCT_BOUNDARY.md" "$PROJECT_DIR/reference/product_boundary.json" "$PROJECT_DIR/reference/SUPPORT_MATRIX.json" "$PROJECT_DIR/reference/DEPENDENCY_POLICY.json" "$PROJECT_DIR/reference/RESOURCE_LIMITS.json" "$PROJECT_DIR/reference/RELAY_OPERATIONS.md" "$PROJECT_DIR/reference/RELEASE_READINESS.md" "$PROJECT_DIR/reference/PUBLIC_RELEASE_REQUIREMENTS.md" "$STAGE/another-dimension-$VERSION/reference/"
...
if [ "${AD_RELEASE_PROFILE:-development}" = "public" ]; then
  PUBLIC_GATE_ARGS="--public-key $AD_RELEASE_PUBLIC_KEY --trust-manifest $AD_RELEASE_TRUST_MANIFEST --trust-manifest-key $AD_RELEASE_TRUST_MANIFEST_KEY --review-bundle $AD_RELEASE_REVIEW_BUNDLE --review-signoff $AD_RELEASE_REVIEW_SIGNOFF --reviewer-public-key $AD_RELEASE_REVIEWER_PUBLIC_KEY"
