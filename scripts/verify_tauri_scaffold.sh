...
require_status_field 'session_durable_state_status' 'read-only durable-state candidate blockers copy'
...
require_contains "$APP_DIR/src/action-state.js" 'then store it and switch'
...
require_contains "$APP_DIR/src/action-state.js" 'manual-pasted-envelope-verify-source'
require_contains "$APP_DIR/src/action-state.js" 'load-remote-envelope-manually'
...
require_contains "$APP_DIR/README.md" 'static blocker copy for the current v0.1 candidate boundary'
