...
require_status_field() {
  ...
  local value="$2"
  local main_value
  main_value="$(printf '%s' "$value" | tr '[:upper:]' '[:lower:]')"

  require_contains "$TAURI_DIR/src/status.rs" "$field: \"$value\""
  require_contains "$APP_DIR/src/main.js" "$main_value"
}
...
require_contains "$APP_DIR/src/production-profile-controller.js" 'loadProductionProfileList'
...
require_contains "$APP_DIR/src/browser-preview-tauri.js" 'production_message_outbound_mark_send_failed'
require_contains "$TAURI_DIR/src/lib.rs" 'production_onion_inbound_envelope_receive_attempt'
require_contains "$TAURI_DIR/src/lib.rs" 'production_onion_descriptor_publication_attempt'
require_contains "$TAURI_DIR/src/lib.rs" 'production_onion_outbound_envelope_send_attempt'
require_contains "$TAURI_DIR/src/lib.rs" 'production_onion_outbound_envelope_send_stored_endpoint_attempt'
...
require_contains "$APP_DIR/src/main.js" 'renderProductionTwoProfileRoomSetupResult'
require_contains "$APP_DIR/src/saved-room-controller.js" 'prepareInviteRoomPrivateRouteExchange'
...
require_contains "$APP_DIR/src/main.js" 'Reply target selected from latest message'
...
require_contains "$APP_DIR/src/main.js" 'isRealOnionRecoveryNoticeKey'
...
require_contains "$APP_DIR/src/i18n.js" 'receiveNeedsVerification'
