...
if [ "${PUBLIC_RELEASE_PREFLIGHT_CHILD:-0}" != "1" ]; then
  ...
  printf '%s\n' "$preflight_output" | grep -Fq -- "real_user_test_allowed_fields=app-version#build-channel#build-commit#platform#public-diagnostics#checksum-result#failure-class#recovery-next-action#desktop-acceptance-status#desktop-acceptance-blockers#app-launch-network" || {
    echo "FAIL release readiness preflight missing real-user allowed fields" >&2
    exit 1
  }
  printf '%s\n' "$preflight_output" | grep -Fq -- "real_user_test_forbidden_fields=raw-logs#endpoints#invite-codes#message-text#local-paths#payloads#safety-phrases#profile-names#passphrases#key-material#private-planning-notes" || {
  ...
  printf '%s\n' "$preflight_output" | grep -Fq -- "next=choose release packaging/upload only after explicit user request, Windows readiness, real-user test preparation, or default transport boundary" || {
