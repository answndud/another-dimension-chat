...
require_text "$ROOT_DIR/scripts/public_release_readiness_preflight.sh" "windows_artifact_type=windows-shell-nsis-exe-installer-candidate"
require_text "$ROOT_DIR/scripts/public_release_readiness_preflight.sh" "windows_runtime_mode=shell-sidecar-pending"
require_text "$ROOT_DIR/scripts/public_release_readiness_preflight.sh" "windows_onion_runtime_compiled=false"
...
require_text "$ROOT_DIR/README.md" "The GitHub Release asset set is current; next owner action is a clean macOS"
require_text "$ROOT_DIR/README.md" "fresh-install run with a disposable profile, followed by representative"
...
require_text "$ROOT_DIR/README.ko.md" "GitHub Release asset set은 current 상태이며, 다음 owner action은 disposable"
require_text "$ROOT_DIR/README.ko.md" "redacted usability evidence가 다음 단계입니다."
...
if [ "${PUBLIC_RELEASE_PREFLIGHT_CHILD:-0}" != "1" ]; then
  ...
  printf '%s\n' "$preflight_output" | grep -Fq -- "stable_candidate_rehearsal=source-only-hold" || {
    echo "FAIL release readiness preflight missing stable candidate rehearsal status" >&2
    exit 1
  }
  printf '%s\n' "$preflight_output" | grep -Fq -- "stable_candidate_rehearsal_generated_artifacts_created=false" || {
    echo "FAIL release readiness preflight missing stable candidate rehearsal artifact boundary" >&2
    exit 1
  }
