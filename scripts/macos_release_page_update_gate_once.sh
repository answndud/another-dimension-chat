...
require_text "$POLICY" "hold body-only edit until proposed live body matches actual live assets"
...
cleanup() {
  rm -f "$live_body" "$live_assets" "$expected_assets" "$missing_assets" "$extra_assets"
}
trap cleanup EXIT
...
if [ -s "$missing_assets" ] || [ -s "$extra_assets" ]; then
  asset_drift=true
  decision="hold-body-only-edit-until-body-and-live-assets-match"
else
  asset_drift=false
...
if [ -s "$missing_assets" ]; then
  printf 'missing_live_assets_from_source_body=\n'
  sed 's/^/- /' "$missing_assets"
fi

if [ -s "$extra_assets" ]; then
  printf 'extra_live_assets_not_in_source_body=\n'
  sed 's/^/- /' "$extra_assets"
fi
