...
require_text "$RELEASE_POLICY" "current source packet upload is held without explicit upload approval"
  ...
  require_text "$FRESH_INSTALL" "clean_install_schema_version=1"
  require_text "$FRESH_RESULT" "Status: hold for manual GUI follow-through; same-release download and checksum passed."
  require_text "$FRESH_RESULT" "clean_install_schema_version=1"
...
for file in "$INSTALL_GUIDE" "$RELEASE_NOTES" "$RELEASE_BODY" "$BETA_CHECKLIST"; do
  ...
  require_text "$file" "artifact_identity=another-dimension-chat-0.1.0-beta-onion-macos-aarch64-unsigned.dmg#ddd48c1316e5eb86ca992d479270d30a151e59839e899949a1055980c4c6bf13#beta-onion#e724bd39#v0.1.0-beta-onion-unsigned#macos-aarch64"
  ...
  require_text "$file" "next_owner_action=$PUBLIC_ARTIFACT_CURRENT_ACTION"
